...
//********************************************************************************
	public static void main(String[] args) throws IOException {
		...
		theTest.reStart();
		System.out.println(theTest.getAllQuestions());
		int choice = 0;
...
//********************************************************************************
				case 2: {
					...
					int serial = theTest.addQuestion(ans, question);
					if (serial == -1) {
					...
					// System.out.println(theTest.getAllQuestions());
					if (ans == 'o' || ans == 'O') { // add open answer
						System.out.println("Type your answer:");
						String answer = input.nextLine();
						theTest.getAllQuestions().get(serial).addAnswer(answer, true);
					} else { // add american answer
						...
						while ((cont == 1) && (num < 10)) {
							...
							theTest.getAllQuestions().get(serial).addAnswer(answer, isCorrect);
					...
					// String a = input.next();
					// System.out
					// .println("This answer 'True' or 'False'?\nIf true - enter T. If false - enter
					// F:");
					// char cor = input.next().charAt(0);
					// boolean isCorrect;
					// while (cor != 't' && cor != 'f' && cor != 'T' && cor != 'F') {
					...
					// isCorrect = false;
					// }
					// theTest.getAmericanQuestions().get(theTest.getAmericanQuestions().size() -
					// 1).addAnswer(a,
					// isCorrect);
...
//********************************************************************************
				case 3: {
					// System.out.println(theTest.printAllQuestions()); *write
					System.out.println("Enter serial of the question you would like to update:");
					int serial = input.nextInt();
					while (!theTest.getAllQuestions().containsKey(serial)) {
					...
					// theTest.editQuestion(newQ,serial); *write

					// System.out.println("In order to edit american question press 'a', for open
					// question press 'o'");
					// char ans = input.next().charAt(0);
...
//********************************************************************************
				case 4: {
					...
					if (theTest.getAllQuestions().get(serial) instanceof OpenQuestion) { // question is open
						System.out.println("Type the new answer:");
						newA = input.nextLine();

					} else { // question is american
						// System.out.println(theTest.printAnswers(serial));
						System.out.println("Enter number of answer you would like to update:");
						index = input.nextInt();
						while (index <= 0
								|| (index > (((AmericanQuestion) (theTest.getAllQuestions().get(serial))).getAnswer())
										.size())) {
						...
						System.out.println("If this answer is correct after update - press 't'. else - press 'f':");
						char cor = input.next().charAt(0);
						while (cor != 't' && cor != 'f' && cor != 'T' && cor != 'F') {
							System.out.println("Please try again");
							cor = input.next().charAt(0);
					...
					// theTest.editAnswer(serial, newA, isCorrect, index);
					...
					// System.out.println("If the answer is correct after update - press 't'. else -
					// press 'f':");
					// char cor = input.next().charAt(0);
					// boolean isCorrect;
					// while (cor != 't' && cor != 'f' && cor != 'T' && cor != 'F') {
...
//********************************************************************************
				case 6: {
					...
					for (int n = 0; n < serialNumbers.length; n++) {
						...
						allQuestNum = theTest.findAmericanQuestion(serialNumbers[n]);
